...
struct MHStoreRelocationServiceTests {
    private enum ValidationError: Error {
        case failed
    }

    @Test
    func relocateIfNeeded_copies_legacy_main_and_sidecars() throws {
    ...
    @Test
    func relocateIfNeeded_overwrites_current_files_and_removes_stale_sidecars() throws {
        let fileManager: FileManager = .default
        let sandboxURL = try makeSandboxDirectory(fileManager: fileManager)
        defer {
            try? fileManager.removeItem(at: sandboxURL)
    ...
    @Test
    func relocateIfNeeded_runs_validation_hook_and_rolls_back_copied_files_on_failure() throws {
        let fileManager: FileManager = .default
        let sandboxURL = try makeSandboxDirectory(fileManager: fileManager)
        defer {
            try? fileManager.removeItem(at: sandboxURL)
        ...
        let validationRecorder = MHStoreRelocationValidationRecorder()

        do {
            _ = try MHStoreRelocationService.relocateIfNeeded(
                plan: .init(
                    legacyStoreURL: storeURLs.legacyStoreURL,
                    currentStoreURL: storeURLs.currentStoreURL
                ),
                fileManager: fileManager
            ) { currentStoreURL, copiedFileNames in
        ...
        expectValidationSnapshot(
            validationRecorder.snapshot(),
            currentStoreURL: storeURLs.currentStoreURL,
            storeFileName: storeFileName
        )
        #expect(
    ...
    @Test
    func relocateIfNeeded_skips_when_locations_match() throws {
        let fileManager: FileManager = .default
        let sandboxURL = try makeSandboxDirectory(fileManager: fileManager)
        defer {
            try? fileManager.removeItem(at: sandboxURL)
    ...
    @Test
    func relocateIfNeeded_skips_when_legacy_store_is_missing() throws {
        let fileManager: FileManager = .default
        let sandboxURL = try makeSandboxDirectory(fileManager: fileManager)
        defer {
            try? fileManager.removeItem(at: sandboxURL)
...
private extension MHStoreRelocationServiceTests {
    struct StoreURLs {
    ...
    func createLegacyStoreFiles(
        ...
        for sidecar in sidecars {
            #expect(
                fileManager.createFile(
                    atPath: legacyDirectoryURL.appendingPathComponent("\(storeFileName)-\(sidecar)").path,
                    contents: Data()
                )
            )
        }
    ...
    func expectedFileNames(
        ...
        ([storeFileName] + sidecars.map { "\(storeFileName)-\($0)" }).sorted()
    }

    func expectValidationSnapshot(
        _ snapshot: MHStoreRelocationValidationRecorder.Snapshot,
        currentStoreURL: URL,
        storeFileName: String
    ) {
        #expect(snapshot.storeURL == currentStoreURL)
        #expect(snapshot.copiedFileNames == expectedFileNames(for: storeFileName, sidecars: ["wal"]))
    }
    ...
    func sortedDirectoryNames(
        ...
        try fileManager.contentsOfDirectory(atPath: directoryURL.path).sorted()
    }
}
