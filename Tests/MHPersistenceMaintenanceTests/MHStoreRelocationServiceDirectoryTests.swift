...
struct MHStoreRelocationServiceDirectoryTests {
    @Test
    func relocateIfNeeded_ignores_matching_directories() throws {
...
private extension MHStoreRelocationServiceDirectoryTests {
    struct Fixture {
        let sandboxURL: URL
        ...
        var expectedStoreFileNames: [String] {
            ["\(storeFileName)-wal", storeFileName].sorted()
        }
    }
    ...
    func assertRelocationResult(
        _ result: MHStoreRelocationOutcome,
