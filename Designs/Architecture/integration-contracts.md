...
- Foundation values:
  - collections and optional collections
  - `Date`, `DateFormatter.Template`, `Calendar`
  - `String`, `StringProtocol`, `Decimal`, and numeric values
- SwiftData values when SwiftData helpers are used:
...
- Collection and optional helpers:
  - `Collection.empty`
...
- Date and calendar helpers:
  ...
  - `DateFormatter.default(_:locale:)`
  - `DateFormatter.fixed(_:)`
  - `Date.stringValue(_:locale:)`
  - `Date.stringValueWithoutLocale(_:)`
  - `String.dateValueWithoutLocale(_:)`
  - `Calendar.utc`
  - `Calendar.endOfDay(for:)`
  - `Calendar.startOfMonth(for:)`
  - `Calendar.endOfMonth(for:)`
  - `Calendar.startOfYear(for:)`
  - `Calendar.endOfYear(for:)`
  - `Calendar.shiftedDate(componentsFrom:in:)`
- String and number helpers:
  - `String.isEmptyOrDecimal`
  - `String.decimalValue`
  - `StringProtocol.normalizedContains(_:)`
  - `Numeric.isZero`
  - `Numeric.isNotZero`
  - `Decimal.isPlus`
  - `Decimal.isMinus`
- SwiftData helpers:
  - `MHPersistentIdentifierCodec`
  - `MHPersistentIdentifierCodecError`
  - `PersistentIdentifier.init(base64Encoded:)`
  - `PersistentIdentifier.base64Encoded()`
  - `ModelContext.fetchFirst(_:)`
  - `ModelContext.fetchRandom(_:)`
  - `ModelContext.fetchRandom(_:using:)`

### Threading / Actor

- Foundation helpers create fresh formatter instances and do not share mutable
  formatter state.
- SwiftData helpers run on the caller's current `ModelContext` actor/threading
  context and do not introduce their own actor isolation.
- `MHPersistentIdentifierCodec` uses local JSON encoder/decoder instances.

### Intended Call Sites

- Shared app libraries that import `MHPlatformCore`
- App targets that import the full `MHPlatform` umbrella
- Widget, Watch, intent, notification, and routing adapters that need stable
  non-UI utility behavior without depending on SwiftUtilities
- SwiftData route/payload/entity identity code that already owns the model
  schema and fetch descriptors

### Ownership Rules (Normative)

- This module is limited to non-domain, non-UI utilities that are stable across
  MHPlatform adopters.
- SwiftUI view modifiers, layout scales, image helpers, color generation, and
  close-button components remain outside MHPlatform and should be evaluated in
  MHUI.
- `PersistentModel.delete()` is intentionally not provided because deletion
  policy and guardrails belong to the app's domain or persistence service.
- `ModelContext` helpers do not create a repository layer; callers still own
  schemas, descriptors, validation, save timing, and fetch policy.
