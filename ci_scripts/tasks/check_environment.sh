...
check_swiftlint_environment() {
  ensure_command "swift" "Install Xcode and ensure SwiftPM is available from the command line."

  if [[ ! -f "Package.swift" ]]; then
    record_failure "Missing file: Package.swift"
    record_next_step "Restore Package.swift so the repository-managed SwiftLint plugin can be resolved."
    return 0
  fi

  if ! grep -q --fixed-strings "https://github.com/SimplyDanny/SwiftLintPlugins" Package.swift; then
